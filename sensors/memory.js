  ...
  poll () {
    ...
    if (plugin.isLinux) {
      child.exec('free -m', (err, stdout, stderr) => {
        ...
        plugin.available = parseInt(data[6])
        const used = parseInt(data[2], 10)
        const total = parseInt(data[1], 10)
        plugin.currentValue = computeUsage(used, total)
        const dataSwap = stdout.split('\n')[2].replace(/[\s\n\r]+/g, ' ').split(' ')
        const usedSwap = parseInt(dataSwap[2], 10)
        const totalSwap = parseInt(dataSwap[1], 10)
        plugin.swapValue = computeUsage(usedSwap, totalSwap)
      })
